...
#### 1.5.1 端口类型区分（最常见错误来源）

**必须明确区分以下两种端口：**

| 端口类型 | 参数/变量 | 说明 |
|---------|----------|------|
| **HTTP 端口** | `--port` / `WEAPP_PORT` | 用于 CLI 和 HTTP API 通信，通过 `cli open --port` 指定 |
| **自动化端口** | `--auto-port` | 用于自动化测试 WebSocket 连接，通过 `cli auto --auto-port` 指定 |

**关键约束**:
- `weapp_open_tool` 和 `weapp_open_project` 返回的是 **HTTP 端口**，不是自动化端口
- `weapp_auto` 的 `autoPort` 参数指定的是 **自动化端口**，不是 HTTP 端口
- 两者完全不同，不可混淆

#### 1.5.2 实例唯一性

微信开发者工具同一时间只能有一个实例运行：
- 启动新实例前必须先关闭现有实例
- `weapp_open_tool` 和 `weapp_open_project` 会自动处理此逻辑
- 关闭操作可能有 3 秒延迟（由微信开发者工具决定）
...
- MD5 计算: `MD5(`${installPath}${nwVersion}`)`

#### 1.5.6 CLI vs HTTP API 选择

| 场景 | 推荐方式 | 原因 |
|------|----------|------|
| 启动/关闭工具 | CLI | 更可靠，直接控制进程 |
| 开启自动化 | CLI | 需要传递 --auto-port 等参数 |
| 预览/上传 | CLI (默认) | 更稳定，输出更详细 |
...
任务执行必须开始前登记，按照规范执行(提示，执行中推荐使用agent swarm)，执行后必须更新交接本并提交git。
