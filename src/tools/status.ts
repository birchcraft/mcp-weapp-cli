  ...
  for (let port = startPort; port <= endPort; port++) {
    checkPromises.push(
      checkPortListening(port).then((isListening) => {
        if (isListening) listeningPorts.push(port);
      })
    );
  }
  ...
  try {
    if (process.platform === 'win32') {
      ...
      for (const line of lines) {
        ...
        if (parts.length >= 2) processes.push({ pid: parts[1].trim() });
      }
 ...
 * 状态检测工具
 */
const weappStatus = createTool<{
  checkHttpPort?: boolean;
  checkAutoPort?: number;
  scanPortRange?: { start: number; end: number };
}>('weapp_status', '检测微信开发者工具的运行状态、端口信息和进程情况')
  .booleanParam('checkHttpPort', '是否检测 HTTP Server 端口（默认 true）')
  .numberParam('checkAutoPort', '要检测的自动化端口（如 9420）')
  .setHandler(async (args) => {
    ...
    try {
      ...
      outputLines.push('', '📊 进程信息:', `   运行中的实例数: ${processes.length}`);
      ...
      if (args.checkHttpPort !== false) {
        ...
        if (httpPorts.length > 0) {
          ...
          for (const port of httpPorts.slice(0, 3)) {
            ...
            if (isWeapp) outputLines.push(`   ✅ 端口 ${port} 可能为微信开发者工具 HTTP Server`);
          }
        } else {
      ...
      if (args.checkAutoPort) {
        ...
        outputLines.push(
          isListening ? `   ✅ 端口 ${args.checkAutoPort} 正在监听 (WebSocket)` : `   ❌ 端口 ${args.checkAutoPort} 未监听`
        );
      }
      ...
      if (args.scanPortRange) {
        ...
        outputLines.push(autoPorts.length > 0 ? `   发现监听端口: ${autoPorts.join(', ')}` : '   未发现监听端口');
      }
      ...
      try {
        ...
        outputLines.push(`   ❌ CLI 连接错误: ${String(cliError)}`);
      }
      ...
      return text(`❌ 状态检测失败: ${String(error)}`, true);
    }
  })
  .build();
 ...
 * 关闭所有实例工具
 */
const weappKillAll = createTool('weapp_kill_all', '强制关闭所有微信开发者工具实例（用于解决多实例冲突问题）')
  .setHandler(async () => {
    ...
    try {
      ...
      return text(`❌ 关闭实例失败: ${String(error)}`, true);
    }
  })
  .build();
 ...
 * 状态检测工具数组
 */
export const statusTools: ToolDefinition[] = [weappStatus, weappKillAll];
